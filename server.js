...
const app = express();
const { CLIENT_ID, PRIVATE_KEY, private_key_id, PORT, MONGODB_URL } =
  process.env;
///////init admin
...
admin.initializeApp({
  credential: admin.credential.cert({
    ...
    client_email:
      "firebase-adminsdk-hknvv@fortune-diary.iam.gserviceaccount.com",
    client_id: CLIENT_ID,
    auth_uri: "https://accounts.google.com/o/oauth2/auth",
    token_uri: "https://oauth2.googleapis.com/token",
    auth_provider_x509_cert_url: "https://www.googleapis.com/oauth2/v1/certs",
    client_x509_cert_url:
      "https://www.googleapis.com/robot/v1/metadata/x509/firebase-adminsdk-hknvv%40fortune-diary.iam.gserviceaccount.com",
  }),
});
...
app.get("/diary/:id", async (req, res) => {
  try {
    const entry = await Diary.find({ _id: req.params.id });
    res.json(entry);
  } catch (error) {
    console.error(error);
    res.status(400).json(error);
